    ...
    Text_decl(args, ret) ->
      Lprim (Pmakeblock (Obj.object_tag, Immutable),
        [Lconst (Const_base (Const_string (name, None)));
         Lprim (prim_fresh_oo_id, [Lconst (Const_base (Const_int 0))], Location.none)], Location.none)
...
and transl_structure loc fields cc rootpath final_env = function
  ...
  | item :: rem ->
      ...
      | Tstr_value(rec_flag, pat_expr_list) ->
          ...
          let body, size =
            transl_structure loc ext_fields cc rootpath final_env rem in
      ...
      | Tstr_include incl ->
          ...
          let rec rebind_idents pos newfields = function
            ...
            | id :: ids ->
                ...
                in
                Llet(Alias, id, Lprim(Pfield pos, [Lvar mid], incl.incl_loc), body), size
          in
  ...
  let rec transl_store rootpath subst = function
    ...
    | item :: rem ->
        ...
        | Tstr_exception ext ->
            ...
            Lsequence(Llet(Strict, id, subst_lambda subst lam,
        ...
        | Tstr_include incl ->
            ...
            let rec store_idents pos = function
                [] -> transl_store rootpath (add_idents true ids subst) rem
              | id :: idl ->
                  Llet(Alias, id, Lprim(Pfield pos, [Lvar mid], loc),
                    Lsequence(store_ident loc id, store_idents (pos + 1) idl))
            in
            Llet(Strict, mid,
       ...
       Llet (Strict, blk,
             ...
             make_sequence
               (fun pos id ->
                 Lprim(Psetfield(pos, Pointer, Initialization),
                       [Lprim(Pgetglobal target_name, [], Location.none);
                        Lprim(Pfield pos, [Lvar blk], Location.none)], Location.none))
               0 pos_cc_list))
  (*
              (* ignore id_pos_list as the ids are already bound *)
